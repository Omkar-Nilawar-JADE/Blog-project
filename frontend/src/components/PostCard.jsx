...
const PostCard = ({ post, idx, bgColor = 'bg-[#f8c291]' }) => {
  const timeAgo = formatDistanceToNow(new Date(post.created_at), { addSuffix: true });

  return (
    <Link to={`/post/${post.id}`}>

      <div className={`relative ${bgColor} border-2 border-black shadow-[4px_4px_0_#000] rounded-lg p-4 w-[300px] flex flex-col justify-between`}>
        <div>
          <h2 className="text-lg font-bold mb-2 leading-snug">{post.title}</h2>
          {post.body && <p className="text-sm mb-4">{post.description}</p>}
        </div>
