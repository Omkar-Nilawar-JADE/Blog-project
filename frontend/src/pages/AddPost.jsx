...
const AddPost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { title, body, description, category } = formData;
  
    // Check for character limits
    if (title.length > 50 || body.length > 700 || description.length > 100) {
    ...
    if (action === "publish") {
      if (!title || !body || !category) {
        alert("Please fill in all required fields to publish the post.");
        return;
      }
      ...
      <div className="max-w-5xl w-full mx-auto">
        ...
        <div className="w-full max-w-xl mx-auto bg-yellow-50 p-8 rounded-lg shadow-2xl border-4 border-black border-solid relative my-auto">
          ...
          <input
            ...
            />
            <p className="text-xs text-gray-700 text-right">{formData.title.length}/50</p>
            ...
            <textarea
              ...
              rows={4}
            />
            <p className="text-xs text-gray-700 text-right">{formData.body.length}/700</p>
            ...
            <textarea
              ...
              rows={2}
            />
            <p className="text-xs text-gray-700 text-right">{formData.description.length}/100</p>
